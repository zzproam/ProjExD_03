...
class Bird:
    ...
    def __init__(self, num: int, xy: tuple[int, int]):
        ...
        self.rct = self.imgs[(+5, 0)].get_rect()
        self.rct.center = xy
    ...
    def update(self, key_lst: list[bool], screen: pg.Surface):
        ...
        sum_mv_tup = tuple(sum_mv)
        if sum_mv_tup in self.imgs:
            screen.blit(self.imgs[(sum_mv_tup)], self.rct)
        else:
            screen.blit(self.imgs[(+5,0)], self.rct)
...
class Bomb:
    ...
    def __init__(self):
        ...
        引数2 rad：爆弾円の半径
        """
        colors = [(255,0,0),(255,255,0),(127,0,255),(255,0,127)]
        directions = [(+5, +5),(+5, -5),(-5, +5),(-5, -5)]
        sizes = [10,20,30,40]
        rad = random.choice(sizes)
